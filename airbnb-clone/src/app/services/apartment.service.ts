...
export class ApartmentService {
  ...
  editApartmentById(id: number, body: iApartment): Observable<iApartment> {
    return this.http.put<iApartment>(`${this.apartmentsUrl}/${id}`, body).pipe(
      catchError((error) => {
        return throwError(() => {
          let message = '';
          if (error.status >= 400 && error.status < 500) {
  ...
  changeAvailability(apartment: iApartment) {
    return this.http
      .put<iApartment>(`${this.apartmentsUrl}/${apartment.id}`, apartment)
      .pipe(
        catchError((error) => {
          return throwError(() => {
            let message = '';
